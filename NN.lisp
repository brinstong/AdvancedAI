#|
Mail me your implementation of this file, and a project report detailing experiments you
did changing various parameters (initial weights, alpha, hidden units, etc., etc.)
and their effects on success or failure to converge to a solution.  It'd be fun to also
do something interesting with this.  For example:

	- modify the code to include a bias unit at the hidden-unit level
	- try some regression problems (see the UCI Machine Learning Repository)
          (I might add some myself if I have time)
	- try some different classification problems (see UCI again)
	- change the algorithm to add Momentum (look it up!) or automatially figure out
	  a good number of hidden units, or experiment with a different function
	  than sigmoid, etc.
	- dynamically adjusting alpha
	- playing with alpha's fixed value
	- changing the number of hidden units
	- adding a touch of random noise to the weights every time you update,
 	  for the first N updates
	- running multiple times
	- using arrays rather than lists for your matrices
	- using a different approach to classification.  Instead of mapping five classes
	  (sean, red, blue, green, bob) to the values 0.1, 0.3, 0.5, 0.7, and 0.9 (say),
	  your output could be a five-dimensional vector, where the classes are mapped to
	  [.9,.1,.1,.1,.1], [.1,.9,.1,.1,.1], [.1,.1,.9,.1,.1], [.1,.1,.1,.9,.1], and
	  [.1,.1,.1,.1,.9].  The idea is now you don't have an arbitrary mapping where 
	  sean is closer to red than green is; instead each class is a corner in a hypercube
	  and they are all the same distance from one another.

See the questions at the end of the file.  Note that there is one "extra credit" implementation function.
...
can be matrixified simply 147351as:
...
1. Your working code
2. An AT LEAST 500-word report on getting the code to work on the test
problems, including tweaks you added and lessons learned.   In LaTeX preferably -- 
you are after all graduate students.  At any rate, no Microsoft Word.
...
(defun net-error (output correct-output)
  ...
  ;; We utilize the mean square error metric to calculate the error
  ;; We extend the formula since we have only 2 dimensions

  (caar (scalar-multiply 0.5 (multiply (transpose (subtract correct-output output))  (subtract correct-output output))))
...
the datum as input."
  ...
  (let
        ((h (map-m 'sigmoid (multiply v (car datum)))))
        (map-m 'sigmoid (multiply w h))
    )
  )
...
returning a list consisting of new, modified V and W matrices."
  ...
  (let* ((i (first datum))
	 (c (second datum)) 
         (h (map-m 'sigmoid (multiply v i))) 
         (o (map-m 'sigmoid (multiply w h)))
         ...
         (v (add v (scalar-multiply alpha (multiply hdelta (transpose i))))))
...
and the final W matrix of the learned network."
  ...
  (let ((lst-of-vw 
            (list (make-random-matrix num-hidden-units (length (caar data)) initial-bounds) (make-random-matrix (length (cdar data)) num-hidden-units initial-bounds)
            )
        ))            
    
        (dotimes (i max-iterations lst-of-vw) 

	  (let ((shuffled-list (shuffle data)))
	    (dolist (d shuffled-list)
                    (setf lst-of-vw (back-propagate d alpha (first lst-of-vw) (second lst-of-vw)))
                )
		...
		(let ((worst-err 0)
		  ...
		  (dolist (d data)
		    (setf cur-err  (optionally-print (net-error (forward-propagate d (first lst-of-vw) (second lst-of-vw)) (second d)) print-all-errors))
		    (setf worst-err (max cur-err worst-err))
                    (setf mean-err (+ mean-err cur-err)))
		  

		  (format t "~%iteration: ~a" i)

		  (format t "~%mean error: ~a" (/ mean-err (length data)))

		  (format t "~%worst error: ~a" worst-err)

		  (if (< worst-err *a-good-minimum-error*)
		   (return lst-of-vw)
		   )
...
and use a modulo of MAX-ITERATIONS."
    ...
    (let* ((data-scaled (scale-data data))
           (converted-scaled-data (convert-data data-scaled))
           (train (butlast converted-scaled-data (floor (length converted-scaled-data) 2)))
           (test (last converted-scaled-data (floor (length converted-scaled-data) 2)))
           (vw (net-build train num-hidden-units alpha initial-bounds max-iterations max-iterations nil)))
       
      (let ((error-sum 0)
	    h
	    o)
	  (dolist (test-input test)
	    (setf h (map-m 'sigmoid (multiply (car vw) (car test-input))))
	    (setf o (map-m 'sigmoid (multiply (second vw) h)))
	    (setf error-sum (+ error-sum (/ (net-error o (second test-input)) (length test))))
	    )
	  error-sum
	  )
	)
...
and use a modulo of MAX-ITERATIONS."
  ...
  (let* ((data-scaled (scale-data data))
         ...
         (err-full-avg 0))
            ...
            (dotimes (times k)
                (let* ((test (subseq converted-scaled-data (floor (* times (length converted-scaled-data)) k) (floor (* (+ times 1) (length converted-scaled-data)) k)))
                       (train (reverse (set-difference converted-scaled-data test :test #'equalp)))
                       (vw (net-build train num-hidden-units alpha initial-bounds max-iterations max-iterations nil)))

		  (let ((err-part-avg 0)
			h
			o)
                            (dolist (test-datum test err-part-avg)
                                (setf h (map-m 'sigmoid (multiply (car vw) (first test-datum))))
                                (setf o (map-m 'sigmoid (multiply (second vw) h))) 
                                (setf err-part-avg (+ err-part-avg (/ (net-error o (second test-datum)) (length test))))
                            )
                            (setf err-full-avg (+ err-full-avg (/ err-part-avg k)))
                        )
                )
            )
            err-full-avg
...
(defparameter *mpg*
  '(((8 307.0 130.0 3504 12.0 70 1) (18.0))    ;;  chevrolet chevelle malibu
    ...
    ((4 97.0 46.0 1835 20.5 70 2) (26.0))    ;;  volkswagen 1131 deluxe sedanmap-m
    ...
    ((8 304.0 150.0 3892 12.5 72 1) (15.0))    ;;  amc matador (sw)map-m
    ...
    ((8 350.0 180.0 3664 11.0 73 1) (11.0))    ;;  oldsmobile omegamap-m
    ...
    ((4 115.0 95.0 2694 15.0 75 2) (23.0))    ;;  audi 100lsmap-m
    ...
    ((8 400.0 180.0 4220 11.1 77 1) (16.0))    ;;  pontiac grand prix ljmap-m
    ...
    ((6 200.0 85.0 2990 18.2 79 1) (19.8))    ;;  mercury zephyr 6map-m
...
;(simple-generalization *voting-records* 3 1.0 5 5000)  ;; pick appropriate values

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
;(simple-generalization *mpg* 3 1.0 5 5000) ;; pick appropriate values
